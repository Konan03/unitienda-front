<template>
  <v-container>
    ...
    <v-row v-if="clothess.length === 0 && !loading">
    ...
    <!-- Lista de productos -->
    <v-row v-else>
      <v-col cols="12" md="3" v-for="clothes in clothess" :key="clothes.id">
        <CardClothesCategory :product="clothes" :loading="loading" />
      </v-col>
    </v-row>
  </v-container>
</template>
...
import CardClothesCategory from '@/components/cardsCategory/CardClothesCategory.vue';
import {
  filtrarPorColores,
  filtrarConDescuento,
  ordenarPorPrecioAsc,
...
const loadProducts = async () => {
  loading.value = true;
  try {
    const response = await getProductosPorCategoria("Ropa");
    clothess.value = response;
  } catch (error) {
...
const applyFilters = async () => {
  ...
  try {
    ...
    if (selectedColors.value.length > 0) {
      clothess.value = await filtrarPorColores(selectedColors.value, "Libros");
    } else {
      await loadProducts();
    }
    ...
    if (selectedPriceRange.value) {
      ...
      clothess.value = allInRange.filter(product => product.categoria === "Libros");
    }
  } catch (error) {
...
const applySort = async (sortType) => {
  ...
  try {
    ...
    clothess.value = sortedProducts.filter(product => product.categoria === "Libros");
  } catch (error) {
