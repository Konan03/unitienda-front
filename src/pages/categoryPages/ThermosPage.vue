<template>
  <v-container>
    ...
    <v-row v-if="thermos.length === 0 && !loading">
    ...
    <!-- Lista de productos -->
    <v-row v-else>
      <v-col cols="12" md="3" v-for="thermo in thermos" :key="thermo.id">
        <CardThermosCategory :product="thermo" :loading="loading" />
      </v-col>
    </v-row>
  </v-container>
</template>
...
import CardThermosCategory from '@/components/cardsCategory/CardThermosCategory.vue';
import {
  filtrarPorColores,
  filtrarConDescuento,
  ordenarPorPrecioAsc,
...
const loadProducts = async () => {
  loading.value = true;
  try {
    const response = await getProductosPorCategoria("Termos");
    thermos.value = response;
  } catch (error) {
...
const applyFilters = async () => {
  ...
  try {
    ...
    if (selectedColors.value.length > 0) {
      thermos.value = await filtrarPorColores(selectedColors.value, "Libros");
    } else {
      await loadProducts();
    }
    ...
    if (selectedPriceRange.value) {
      ...
      thermos.value = allInRange.filter(product => product.categoria === "Libros");
    }
  } catch (error) {
...
const applySort = async (sortType) => {
  ...
  try {
    ...
    thermos.value = sortedProducts.filter(product => product.categoria === "Libros");
  } catch (error) {
