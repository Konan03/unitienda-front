<template>
  <v-container>
    ...
    <v-row v-if="stationerys.length === 0 && !loading">
    ...
    <!-- Lista de productos -->
    <v-row v-else>
      <v-col cols="12" md="3" v-for="stationery in stationerys" :key="stationery.id">
        <CardStationeryCategory :product="stationery" :loading="loading" />
      </v-col>
    </v-row>
  </v-container>
</template>
...
import CardStationeryCategory from '@/components/cardsCategory/CardStationeryCategory.vue';
import {
  filtrarPorColores,
  filtrarConDescuento,
  ordenarPorPrecioAsc,
...
const loadProducts = async () => {
  loading.value = true;
  try {
    const response = await getProductosPorCategoria("Papeleria");
    stationerys.value = response;
  } catch (error) {
...
const applyFilters = async () => {
  ...
  try {
    ...
    if (selectedColors.value.length > 0) {
      stationerys.value = await filtrarPorColores(selectedColors.value, "Libros");
    } else {
      await loadProducts();
    }
    ...
    if (selectedPriceRange.value) {
      ...
      stationerys.value = allInRange.filter(product => product.categoria === "Libros");
    }
  } catch (error) {
...
const applySort = async (sortType) => {
  ...
  try {
    ...
    stationerys.value = sortedProducts.filter(product => product.categoria === "Libros");
  } catch (error) {
